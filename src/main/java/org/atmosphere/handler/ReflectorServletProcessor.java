...
/**
 * Simple {@link AtmosphereServletProcessor} that delegate the
 * {@link AtmosphereHandler#onRequest} to a set of {@link FilterChain} and
 * {@link Servlet#service(javax.servlet.ServletRequest,
 * javax.servlet.ServletResponse)
 * and store the {@link AtmosphereResource} as a {@link HttpServletRequest}
 * attribute named org.atmosphere.cpr.AtmosphereResource. The
 * {@link AtmosphereResource} can later be retrieved and used to supend/resume
 * and broadcast
 * @author Jeanfrancois Arcand
 */
public final class ReflectorServletProcessor extends
        AbstractReflectorAtmosphereHandler implements
        AtmosphereServletProcessor {

    private final static String APPLICATION_NAME = "applicationClassName";
    private static final Logger logger = LoggerFactory
            .getLogger(ReflectorServletProcessor.class);

    private String servletClassName;
    private final ArrayList<String> filtersClass = new ArrayList<String>();
    private final ArrayList<FilterAndName> filters = new ArrayList<FilterAndName>();
    ...
    public ReflectorServletProcessor(final Servlet servlet) {
        this.servlet = servlet;
    }

    void loadWebApplication(final ServletConfig sc)
            throws MalformedURLException, InstantiationException,
            IllegalAccessException, ClassNotFoundException {

        URL url = sc.getServletContext().getResource("/WEB-INF/lib/");
        URLClassLoader urlC = new URLClassLoader(new URL[] {url}, Thread
                .currentThread().getContextClassLoader());

        if (getServletClassName() != null && servlet == null) {
            try {
                servlet = (Servlet) urlC.loadClass(getServletClassName())
                        .newInstance();
            } catch (NullPointerException ex) {
                // We failed to load the servlet, let's try directly.
                servlet = (Servlet) Thread.currentThread()
                        .getContextClassLoader().loadClass(servletClassName)
                        .newInstance();
            }
        }
    ...
    public void addFilter(final Filter f, final String filterName) {
        filters.add(new FilterAndName(f, filterName));
    }

    public void addFilter(final FilterAndName fan) {
        filters.add(fan);
    }

    /**
     * Delegate the request to the Servlet.service method, and add the
     * {@link AtmosphereResource} to the
     * {@link HttpServletRequest#setAttribute(java.lang.String, java.lang.Object)
     * )}. The {@link AtmosphereResource} can ve retrieved using
     ...
     * @param servletClass the servletClass to set
     */
    public void setServletClassName(final String servletClass) {
        this.servletClassName = servletClass;
    }
